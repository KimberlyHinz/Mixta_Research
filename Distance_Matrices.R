...
# NOTE: Project_ELEVEN has been abandonned because none of the Mixta genes were closely related with V. cholerae. BUT, there will still be four 
# sections where two will use the genes that passed through the 90% length filter and the other two will contain the additional 58 genes that 
# passed the second (optional) filter.
...
dist_results <- function(Files, skip_name, skip_mat, saveFolder, Project) {
  ...
  for(row in 1:nrow(Files)) {
    ...
    dist_mat <- subset(dist_mat, select = 2:11)
    
    dist_mat <- data.frame(lapply(dist_mat, gsub, pattern = "\\[|\\]", replacement = "")) # Remove the brackets
    
    for(row2 in 2:(nrow(dist_mat) - 1)) {                                          # Shift over distance cells so SE are separate
      for(col in 1:(row2 - 1)) {
    ...
    results <- data.frame(gene = Files$Gene[row],
                          ...
                          cal_dist = c(t(dist_mat[5, 1:5]), dist_mat[6:10, 5]),
                          cal_SE = c(dist_mat[1:5, 5], t(dist_mat[5, 6:10])),
                          gav_dist = c(t(dist_mat[6, 1:6]), dist_mat[7:10, 6]),
                          gav_SE = c(dist_mat[1:6, 6], t(dist_mat[6, 7:10])))
...
dist_results(Files = megFiles, skip_name = 34, skip_mat = 47,
...
dist_results(Files = megFiles, skip_name = 33, skip_mat = 45,
             saveFolder = "8_Results_Ten_AA/", Project = "Ten_AA")

rm(megFiles)
#
# Project_TEN w/ Genes that had Longer Sequences -------------------------------------------------------------------------------------------------
## Nucleotides w/ Extra ==========================================================================================================================
...
megFiles <- data.frame(File_name = list.files(path = "7_Ext_Distance_Ten_NT/",
                                              pattern = ".meg"))                          # Reads in the dist mat (meg) file names as a dataframe
megFiles <- mutate(megFiles,
                   Path_name = paste("7_Ext_Distance_Ten_NT", File_name, sep = "/"),
                   Gene = gsub(pattern = "-19020.meg|-7720.meg|-23436.meg|-272.meg|10960.meg|10692.meg|24416.meg", 
                               replacement = "", x = File_name))                          # Adds the gene name (no extension)

dist_results(Files = megFiles, skip_name = 34, skip_mat = 47,
             saveFolder = "8_Ext_Results_Ten_NT/", Project = "Ext_Ten_NT")
...
megFiles <- data.frame(File_name = list.files(path = "7_Ext_Distance_Ten_AA/",
                                              pattern = ".meg"))                          # Reads in the dist mat (meg) file names as a dataframe
megFiles <- mutate(megFiles,
                   Path_name = paste("7_Ext_Distance_Ten_AA", File_name, sep = "/"),
                   Gene = gsub(pattern = "-6676.meg|-10760.meg|-6136.meg|-18080.meg|10792.meg|9556.meg", 
                               replacement = "", x = File_name))                          # Adds the gene name (no extension)

dist_results(Files = megFiles, skip_name = 33, skip_mat = 45,
             saveFolder = "8_Ext_Results_Ten_AA/", Project = "Ext_Ten_AA")
