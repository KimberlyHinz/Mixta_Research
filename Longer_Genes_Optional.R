...
for(row in 1:nrow(uniq_genes)) {
  ...
  if(above_80 == 11) {
   mssg <- check_sequences(above_90, gene_file_org, "4.2_Longer_Eleven_NT", num_rows = 11)
   # count_90 = above_90; gn_fl = gene_file_org; proj_file = "4.2_Longer_Eleven_NT"; num_rows = 11
   print(mssg)
  } else { print("Didn't pass the 80% cutoff")}
}

rm(gene_file_org, above_80, above_90, mssg, row)
rm(longer_genes, uniq_genes)
# 
## ELEVEN - Amino Acids ==========================================================================================================================
uniq_genes <- data.frame(File = list.files(path = "4.2_Longer_Eleven_NT/", 
                                                pattern = ".fasta"))                      # Dataframe containing the fasta gene file names

uniq_genes <- mutate(uniq_genes,
                     Path_name = paste("3_Homologous_Eleven_AA", File, sep = "/"))   # Adds file pathway
...
for(row in 1:nrow(uniq_genes)) {
  ...
  check <- my_write_fasta(gene_file_org, project_file = "4.2_Longer_Eleven_AA")
  print(check)
}
...
# Project_TEN ------------------------------------------------------------------------------------------------------------------------------------
## TEN - Nucleotides =============================================================================================================================
longer_genes <- read.csv(file = "8_Results_Ten_NT/Files_w_Longer_Sequences_175.csv", 
                         stringsAsFactors = FALSE)
...
for(row in 1:nrow(uniq_genes)) {
  ...
  if(above_80 == 10) {
    mssg <- check_sequences(above_90, gene_file_org, "4.2_Longer_Ten_NT", num_rows = 10)
...
#
## TEN - Amino Acids =============================================================================================================================
uniq_genes <- data.frame(File = list.files(path = "4.2_Longer_Ten_NT/", 
                                           pattern = ".fasta"))                      # Dataframe containing the fasta gene file names
...
for(row in 1:nrow(uniq_genes)) {
  ...
  check <- my_write_fasta(gene_file_org, project_file = "4.2_Longer_Ten_AA")
  print(check)
}
